...
export const getAllProduct = async (req, res, next) => {
  try {
    ...
    const apiFeatures = new ApiFeatures(createProductModel.find(), req.query)
      .search()
      .filter()
...
export let updateProduct = async (req, res, next) => {
  try {
    ...
    product = await createProductModel.findByIdAndUpdate(
      req.params.id,
      req.body,
      {
        new: true,
...
export let deleteReviews = async (req, res, next) => {
  try {
    ...
    if (!productId || !reviewId) {
      return next(
        new ErrorHandler("Product ID and Review ID are required", 400)
      );
    }
